 ...
 * This is a port of the PrinterOneDataQueries class which uses the Alfresco Web Services API. This example
...
public class PrinterOneCMISDataQueries extends CMISExampleBase {

	private static final String RESULT_SEP = "======================";
	private static final String USAGE = "java PrinterOneCMISDataQueries <username> <password> <root folder>";
	    
	public static void main(String[] args) {
		if (args.length != 3) doUsage(PrinterOneCMISDataQueries.USAGE);
    	PrinterOneCMISDataQueries sccdq = new PrinterOneCMISDataQueries();
	...
	public void doExamples() {
		...
		System.out.println("Finding content of type:" ); //+ PrinterOneModel.TYPE_SC_DOC.toString()
		queryString = "select * from hysitech:document";
		...
		queryString = "select d.*, w.* from cmis:document as d join printerOneContratos:printerOneContratos as w on d.cmis:objectId = w.cmis:objectId";
		dumpQueryResults(getQueryResults(queryString));
