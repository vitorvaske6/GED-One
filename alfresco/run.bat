...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f gedone-share
	call %MVN_EXEC% clean package -pl gedone-share,gedone-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill gedone-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f gedone-acs
	call %MVN_EXEC% clean package -pl gedone-integration-tests,gedone-platform,gedone-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl gedone-platform,gedone-integration-tests,gedone-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl gedone-platform,gedone-integration-tests
EXIT /B 0
:purge
