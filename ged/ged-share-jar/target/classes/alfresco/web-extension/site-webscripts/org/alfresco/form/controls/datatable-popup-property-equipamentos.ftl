...
<#if field??>
    <label onclick="initDTPequipamentos" for="${fieldHtmlId}">Equipamentos do Contrato:<#if field.mandatory><span
                class="mandatory-indicator">${msg("form.required.fields.marker")}</span><span onclick="initDTPequipamentos()" id="${fieldHtmlId}_show" style="cursor: pointer;"> (Clique aqui para visualizar equipamentos) </span></#if></label>
