...
<script type="text/javascript" src="http://yui.yahooapis.com/2.9.0/build/datatable/datatable-min.js" group="hysitechForm"></script>
...
<script type="text/javascript">//<![CDATA[
    if (window.addEventListener) {
        window.addEventListener('load', function () {
            DTP.init("${fieldHtmlId}", "${dtConfig?html}", "${form.mode}", "${field.label?html}");
        });
        ...
        */
        datatableEvents: function () {
            ...
            if (!DTP.readOnly) {
                ...
                addButton.innerHTML = '+';
                addButton.title = 'Add novo equipamento';
            ...
            if (DTP.readOnly) {
                DTP.dtpDatatable.subscribe('cellClickEvent', function () {
                    DTP.popupDT();
                });
                DTP.tooltip = new YAHOO.widget.Tooltip("dtp-tooltip", {
                    context: "dtp-dt",
        ...
        */
        handleConfirm: function (toDelete) {
            ...
            var handleYes = function () {
                this.hide();
                DTP.dtpDatatable.deleteRow(toDelete);
                DTP.dtToJson();
                return true;
            };
