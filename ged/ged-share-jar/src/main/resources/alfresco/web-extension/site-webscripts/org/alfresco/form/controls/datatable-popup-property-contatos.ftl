...
<#if field??>
    <label onclick="initDTPcontatos" for="${fieldHtmlId}">Contatos do Contrato:<#if field.mandatory><span
                class="mandatory-indicator">${msg("form.required.fields.marker")}</span><span onclick="initDTPcontatos()" id="${fieldHtmlId}_show" style="cursor: pointer;"> (Clique aqui para visualizar os contatos) </span></#if></label>
